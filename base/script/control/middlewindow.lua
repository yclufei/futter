...
MiddleWindow = inherit(GmBase.Window):name("MiddleWindow")

MiddleWindow.init = function (self,html,fmgr)
	local top = self
	local win = self
...
MiddleWindow.on_sys_button = function(self,stype)
	if stype == "min" then
		self:minbox()
...
--设置移动区域
MiddleWindow.set_drag_element = function(self,eid)
	self.drag = self:get_child(eid)
	self.drag:set_drag_window(true)
end

MiddleWindow.addSubMenuLink = function(self,ename)
	local top = self
...
MiddleWindow.MoveFrame = function(self,left,sort,efunc)
...
-- 设置顶层面板
MiddleWindow.addNavFrame = function(self,fname,func,endfunc)	
	local top = self	
	...
	if not self.frameMoving and  self.curframe ~= moveframe then
		...
		self:MoveFrame(true, sort - 1,function(self)
																...
																top:printNav("MiddleWindow.addNavFrame",sort)
		...
		self:printNav("MiddleWindow.addNavFrame",sort)
...
-- 移开顶层面板
MiddleWindow.delTopNavFrame = function(self,fname)	
	local top = self
	local sort = table.size(self.mNav)
	if  not self.frameMoving then 
		...
		if fname == nil then
			print("MiddleWindow.delTopNavFrame",fname,sort)
			self:MoveFrame(false, sort - 1,function(self)
																			...
																			top:updatetick(true)
																			top:printNav("MiddleWindow.delTopNavFrame")
																			top.frameMoving = false
																 end)
...
-- 直接显示面板
MiddleWindow.showFrame = function(self,fname,func)

	print('MiddleWindow.showFrame ',fname,func)
	...
	if not self.frameMoving and moveframe ~= nil and self.curframe ~= moveframe then
		...
		self:printNav("MiddleWindow.showFrame")
...
-- 显示弹出层
MiddleWindow.pushFrame = function(self,fname,func)
	local top = self	
	local moveframe = self:get_frame(fname,func)	
	local sort = table.size(self.mNav)		
...
-- 隐藏弹出层
MiddleWindow.popFrame = function(self)
	local top = self	
	local sort = table.size(self.mNav)
	self.curframe:show2(false,false)
	self.mNav[sort] = nil	
...
MiddleWindow.show_popup = function(self,fname,data)
	self.curPop = fname
...
MiddleWindow.hide_popup = function(self)
	self:layout()
