...
categarylistMiddleArea = inherit(GmCtrl.ControlBase):name("categarylistMiddleArea")

categarylistMiddleArea.init = function(self,parent,fname)
...
categarylistMiddleArea.set_data = function(self,data)
	self.data = data
	...
	if data['object'] and table.size(data['object']) > 0 then
		...
		for a,b in pairs(data['object']) do	
			...
			if b['objecttitle'] then
				self.title[osort]:set_inner_text(b['objecttitle'])
			end
			if b['parent'] and b['parent']['title'] then
