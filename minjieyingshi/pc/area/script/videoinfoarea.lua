...
videoinfoareaArea = inherit(GmCtrl.ControlBase):name("videoinfoareaArea")

videoinfoareaArea.init = function (self,parent,aid)

	local top = self
	local fid = aid or "videoinfoareaArea"
	...
	self.thumbpath:set_mousedown(function(ctrl)
		local a,b,c,d = ctrl:get_scroll_pos()
		ctrl.parent:set_scroll_pos(0,b)
		print('set_mousedown',a,b,c,d)
	end)
	...
	self.summary:set_mousedown(function(ctrl)
		local a,b,c,d = ctrl:get_scroll_pos()
		ctrl.parent:set_scroll_pos(0,b)
		print('set_mousedown',a,b,c,d)
	end)
...
videoinfoareaArea.callback = function(self,key,tb)
  ...
  if tb.act == 'prev' and table.eq(attrs,tb) then		
		...
		print('videoinfoareaArea callback error ',key,tb)
	end
end



videoinfoareaArea.set_data = function(self,data)
	self.data = data
	
	if self.videodetail and self.videodetail.set_data then
		self.videodetail:set_data(data)
	end			
