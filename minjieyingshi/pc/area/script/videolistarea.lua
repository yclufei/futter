...
videolistareaArea = inherit(GmBase.objectlistArea):name("videolistareaArea")

videolistareaArea.init = function (self,parent,aid)

	local top = self
	local fid = aid or "videolistareaArea"
	GmBase.objectlistArea.init(self,parent,'tid',20,5)

	
	
	self.mgr = gm_minjieyingshi.clsvideomgr(self.movecount)
...
videolistareaArea.on_show = function(self,key)
	self:init_attrs()
	self:init_net_data()
end


videolistareaArea.on_hide = function(self,key)
	self:clean()
end


videolistareaArea.set_data = function(self,data)
	if data then
		self.data = data
	end
end
...
videolistareaArea.init_attrs = function(self)
...
videolistareaArea.on_scroll_top = function(self,pwidth,pheight,pxx,topY)	
	local top = self
	self:clean()
...
videolistareaArea.alloc_item_ctrl = function(self,idx)
	return gm_minjieyingshi.videoitemItem(self.itemParent,'item'..idx)
end


videolistareaArea.get_prev_local_data = function(self,id,count)
	self:init_attrs()
...
videolistareaArea.get_next_local_data = function(self,id,count)
	self:init_attrs()
	local wherestr = ' where CAST('..self.sorkey..' AS int) < '..tostring(id)
