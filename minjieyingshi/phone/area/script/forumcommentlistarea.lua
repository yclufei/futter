...
forumcommentlistareaArea.init = function (self,parent,aid)
	...
	self.mgr = gm_minjieyingshi.clsforumcommentmgr(self.movecount)
...
forumcommentlistareaArea.alloc_item_ctrl = function(self,idx)
	return gm_minjieyingshi.forumcommentitemItem(self.itemParent,'item'..idx)
end


forumcommentlistareaArea.get_prev_local_data = function(self,id,count)
	self:init_attrs()
...
forumcommentlistareaArea.get_next_local_data = function(self,id,count)
	self:init_attrs()
	local wherestr = ' where CAST('..self.sorkey..' AS int) < '..tostring(id)
