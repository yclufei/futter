...
forumcommentaddareaArea.init = function (self,parent,aid)
	...
	local top = self
	local fid = aid or "forumcommentaddareaArea"
	...
	self.scrollcontainer:set_scroll_container(true)
		...
		-- form.lua.xsl form 87 chayegongsi/classdata/forumcommentpost.xml
	...
	self.mgr = gm_chayegongsi.clsforumcommentmgr.get_instance()
	self.mgr:add_ui('forumcommentaddareaArea',self)
	
		
	self.public = self:get_child('public')
	self.public:set_click(function(link)			
		-- todo
		top.mgr:run('forumcommentpost',top)
	end)
...
forumcommentaddareaArea.callback = function(self,key,tb)
  ...
  if tb.act == 'prev' and table.eq(attrs,tb) then		
		...
		print('forumcommentaddareaArea callback error ',key,tb)
	end
end
