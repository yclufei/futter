...
serviceaddareaArea.init = function (self,parent,aid)
	...
	self.scrollcontainer:set_scroll_container(true)
		...
		-- form.lua.xsl form 87 chayegongsi/classdata/servicepost.xml
	...
	self.mgr = gm_chayegongsi.clsservicemgr.get_instance()
...
serviceaddareaArea.callback = function(self,key,tb)
  ...
  if tb.act == 'prev' and table.eq(attrs,tb) then		
		...
		print('serviceaddareaArea callback error ',key,tb)
	end
end
...
serviceaddareaArea.set_data = function(self,data)
	...
	if self.servicepost and self.servicepost.set_data then
		self.servicepost:set_data(data)
	end			
