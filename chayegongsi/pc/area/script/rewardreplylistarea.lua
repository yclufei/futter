...
rewardreplylistareaArea.init = function (self,parent,aid)
	...
	self.mgr = gm_chayegongsi.clsrewardreplymgr(self.movecount)
...
rewardreplylistareaArea.alloc_item_ctrl = function(self,idx)
	return gm_chayegongsi.rewardreplyitemItem(self.itemParent,'item'..idx)
end


rewardreplylistareaArea.get_prev_local_data = function(self,id,count)
	self:init_attrs()
...
rewardreplylistareaArea.get_next_local_data = function(self,id,count)
	self:init_attrs()
	local wherestr = ' where CAST('..self.sorkey..' AS int) < '..tostring(id)
