...
rewardreplyaddareaArea = inherit(GmCtrl.ControlBase):name("rewardreplyaddareaArea")

rewardreplyaddareaArea.init = function (self,parent,aid)

	local top = self
	local fid = aid or "rewardreplyaddareaArea"
	...
	self.scrollcontainer:set_scroll_container(true)
		...
		-- form.lua.xsl form 87 chayegongsi/classdata/rewardreplypost.xml
	...
	self.mgr = gm_chayegongsi.clsrewardreplymgr.get_instance()
	self.mgr:add_ui('rewardreplyaddareaArea',self)
...
rewardreplyaddareaArea.callback = function(self,key,tb)
...
rewardreplyaddareaArea.set_ref_data = function(self,data)
	table.print(data)
...
rewardreplyaddareaArea.set_data = function(self,data)
	self.data = data
