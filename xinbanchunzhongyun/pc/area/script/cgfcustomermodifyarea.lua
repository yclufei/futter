...
cgfcustomermodifyareaArea = inherit(GmCtrl.ControlBase):name("cgfcustomermodifyareaArea")

cgfcustomermodifyareaArea.init = function (self,parent,aid)

	local top = self
	local fid = aid or "cgfcustomermodifyareaArea"
...
cgfcustomermodifyareaArea.callback = function(self,key,tb)
  ...
  if tb.act == 'prev' and table.eq(attrs,tb) then		
		...
		print('cgfcustomermodifyareaArea callback error ',key,tb)
	end
end



cgfcustomermodifyareaArea.set_data = function(self,data)
	self.data = data
