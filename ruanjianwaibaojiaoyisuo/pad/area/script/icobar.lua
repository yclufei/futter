...
icobarArea = inherit(GmCtrl.ControlBase):name("icobarArea")

icobarArea.init = function (self,parent,aid)

	local top = self
	local fid = aid or "icobarArea"
...
icobarArea.callback = function(self,key,tb)
  ...
  if tb.act == 'prev' and table.eq(attrs,tb) then		
		...
		print('icobarArea callback error ',key,tb)
	end
end



icobarArea.set_data = function(self,data)
	self.data = data
