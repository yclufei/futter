...
productinfoareaArea = inherit(GmCtrl.ControlBase):name("productinfoareaArea")

productinfoareaArea.init = function (self,parent,aid)

	local top = self
	local fid = aid or "productinfoareaArea"
	...
	self.scrollcontainer:set_scroll_container(true)
		...
		self.productdetail = gm_ruanjianwaibaojiaoyisuo.productitemItem(self.scrollcontainer)
...
productinfoareaArea.callback = function(self,key,tb)
  ...
  if tb.act == 'prev' and table.eq(attrs,tb) then		
		...
		print('productinfoareaArea callback error ',key,tb)
	end
end



productinfoareaArea.set_data = function(self,data)
	self.data = data
	
	if self.productdetail and self.productdetail.set_data then
		self.productdetail:set_data(data)
	end			
