...
ranklistinfoareaArea = inherit(GmCtrl.ControlBase):name("ranklistinfoareaArea")

ranklistinfoareaArea.init = function (self,parent,aid)

	local top = self
	local fid = aid or "ranklistinfoareaArea"
	...
	self.scrollcontainer:set_scroll_container(true)
		...
		self.ranklistdetail = gm_ruanjianwaibaojiaoyisuo.ranklistitemItem(self.scrollcontainer)
...
ranklistinfoareaArea.callback = function(self,key,tb)
  ...
  if tb.act == 'prev' and table.eq(attrs,tb) then		
		...
		print('ranklistinfoareaArea callback error ',key,tb)
	end
end



ranklistinfoareaArea.set_data = function(self,data)
	self.data = data
	
	if self.ranklistdetail and self.ranklistdetail.set_data then
		self.ranklistdetail:set_data(data)
	end			
