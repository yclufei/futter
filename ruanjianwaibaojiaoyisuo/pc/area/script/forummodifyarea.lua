...
forummodifyareaArea = inherit(GmCtrl.ControlBase):name("forummodifyareaArea")

forummodifyareaArea.init = function (self,parent,aid)

	local top = self
	local fid = aid or "forummodifyareaArea"
...
forummodifyareaArea.callback = function(self,key,tb)
  ...
  if tb.act == 'prev' and table.eq(attrs,tb) then		
		...
		print('forummodifyareaArea callback error ',key,tb)
	end
end



forummodifyareaArea.set_data = function(self,data)
	self.data = data
