...
circleareaArea.init = function (self,parent,aid)
	...
	self.ref_profile_mgr = gm_ruanjianwaibaojiaoyisuo.clsprofilemgr(self.movecount)
...
circleareaArea.get_ref_data = function(self,data)
	...
	if key then
		if self.cache_refdata[key] == nil then
			...
			local a,b,c = self.ref_mgr:GetSingleRecord()
			
			
			self.ref_profile_mgr:SetDirty()
			self.ref_profile_mgr:SetAttr('uid',key)
			local a1,b1,c1 = self.ref_profile_mgr:GetSingleRecord()	
...
circleareaArea.on_scroll_stop = function(self,pwidth,pheight,pxx,topY)
	print('circleareaArea.on_scroll_stop')
end
