...
-- D:\GOA\reslibrary\templete\default\phone\area\nav\lua.xsl	
...
navArea.init = function (self,parent,aid)
	...
	self.param['leftcss']  = function(top,value)
		top.nvleft:set_class(' NavTDWidth centerbg divcenter padding15  floatL '..value)
	end
	...
	self.param['rightcss']  = function(top,value)
		top.nvright:set_class(' NavTDWidth centerbg divcenter padding15 floatR '..value)
	end
...
navArea.set_data = function(self,tb)
	...
	elseif tb['name'] then
		self.nvtxt:set_inner_text(tostring(tb['name']))
	end
end
